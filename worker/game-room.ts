...
export class GameRoom extends DurableObject<Env> {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === "/create" && request.method === "POST") {
      ...
      this.room = { code: body.code, status: "WAITING", stage: 0, players: [player], hostPlayerId: player.id, hostUserId: body.hostUserId ?? null, attempts: 0, startedAt: null, completedAt: null, optionalEvidence: false, entitlement: Boolean(body.owner), votes: {}, finalAnswers: {} };
      await this.ctx.storage.put("room", this.room);
  ...
  private async applyAction(player: RoomPlayer, action: GameAction): Promise<{ ok: boolean; error?: string; status?: number }> {
    ...
    if (this.room.status === "PAYWALL") return { ok: false, error: "PAYMENT_REQUIRED", status: 402 };

    let correct = false;
    let event = "";
    if (this.room.stage === 1 && action.type === "SUBMIT_CODE" && player.role === "FIELD") { correct = simpleActionIsCorrect(1, player.role, action); if (correct) { this.room.stage = 2; event = "PACKET_DECRYPTED"; } }
    else if (this.room.stage === 2 && action.type === "SUBMIT_CODE" && player.role === "ARCHIVE") { correct = simpleActionIsCorrect(2, player.role, action); if (correct) { this.room.stage = this.room.entitlement ? 4 : 3; this.room.status = this.room.entitlement ? "PLAYING" : "PAYWALL"; event = this.room.entitlement ? "FULL_CASE_STARTED" : "DEMO_COMPLETED"; } }
    ...
    else if (this.room.stage === 5 && action.type === "CONFIRM_DIGIT" && player.role === "FIELD") { correct = simpleActionIsCorrect(5, player.role, action); if (correct) { this.room.stage = 6; event = "FALSE_INSTRUCTION_BROKEN"; } }
    else if (this.room.stage === 6 && action.type === "TRACE_PATH" && player.role === "FIELD") { correct = simpleActionIsCorrect(6, player.role, action); if (correct) { this.room.stage = 7; event = "PATH_RECONSTRUCTED"; } }
    else if (this.room.stage === 7 && action.type === "CALL_DECISION" && player.role === "ARCHIVE") { correct = simpleActionIsCorrect(7, player.role, action); if (correct) { this.room.callPath = value as "ANSWERED" | "DECLINED"; this.room.stage = 8; event = "CALL_COMPLETED"; } }
    ...
    else if (this.room.stage === 9 && action.type === "FINAL_ANSWER" && typeof value === "string") {
      const expected = expectedFinalAnswer(player.role);
      this.room.finalAnswers[player.id] = value; correct = true; event = "ANSWER_LOCKED";
      if (Object.keys(this.room.finalAnswers).length === 2) {
        const room = this.room;
        const allCorrect = room.players.every((member) => room.finalAnswers[member.id] === expectedFinalAnswer(member.role));
